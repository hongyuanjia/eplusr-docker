...
RUN apt-get update \
    && apt-get install -y --no-install-recommends \
        ...
        zlib1g-dev \
    ## Install eplusr
    && Rscript -e "install.packages('eplusr', repos = c(CRAN = 'https://packagemanager.rstudio.com/cran/__linux__/focal/latest'))" \
    ## Install EnergyPlus
    && Rscript -e "eplusr::install_eplus('9.2.0', local = TRUE)"
